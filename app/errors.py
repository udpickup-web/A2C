...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    return JSONResponse(status_code=422, content=error_payload(422, "Validation error", exc.errors()))

async def http_exception_handler(request: Request, exc: StarletteHTTPException):
    # Map Starlette/FastAPI HTTPException into our envelope
    return JSONResponse(status_code=exc.status_code, content=error_payload(exc.status_code, str(exc.detail), {}))
